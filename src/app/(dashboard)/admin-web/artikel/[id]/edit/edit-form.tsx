...
import { updateArtikel } from "@/app/actions/web-admin"
import { useState, useRef } from "react"
import Link from "next/link"
import { ArrowLeft, Image as ImageIcon, Trash } from "lucide-react"
import { DatePicker } from "@/components/ui/date-picker"

export default function EditArtikelForm({ initialData, id }: { initialData: any, id: string }) {
  const [isPending, setIsPending] = useState(false)
  const [gambarPreview, setGambarPreview] = useState<string | null>(initialData?.gambarUrl || null)
  ...
  const handleAction = async (formData: FormData) => {
    setIsPending(true)
    if (gambarPreview === "gambar-removed") {
      formData.set("gambarUrl", "gambar-removed")
    } else if (gambarPreview && gambarPreview.startsWith("data:")) {
      formData.set("gambarUrl", gambarPreview)
    }
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Update berita atau kegiatan BUMDes.</p>
        </div>
      </div>
          ...
          <div className="grid gap-4">
             <div className="grid grid-cols-2 gap-4">
               <div className="space-y-2">
                 ...
                 <Input id="judul" name="judul" required defaultValue={initialData?.judul} />
               </div>
                <div className="space-y-2">
             ...
             <div className="space-y-2">
               ...
               <Select name="kategori" defaultValue={initialData?.kategori || "BERITA"}>
                 <SelectTrigger>
             ...
             <div className="space-y-2">
               ...
               {gambarPreview && gambarPreview !== "gambar-removed" ? (
                 <div className="relative w-full max-w-sm aspect-video rounded-md overflow-hidden border">
                   ...
                   <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2" onClick={() => { setGambarPreview("gambar-removed"); if (fileInputRef.current) fileInputRef.current.value = "" }}>
             ...
             <div className="space-y-2">
               ...
               <Textarea id="ringkasan" name="ringkasan" defaultValue={initialData?.ringkasan || ""} rows={2} />
             </div>

             <div className="space-y-2">
               <Label htmlFor="konten">Isi Lengkap Postingan *</Label>
               <Textarea id="konten" name="konten" required defaultValue={initialData?.konten} rows={8} />
             </div>

             <div className="space-y-2">
               <Label htmlFor="published">Status Publikasi</Label>
               <Select name="published" defaultValue={initialData?.published ? "publik" : "draf"}>
                 <SelectTrigger>
